...
class GenerateFoundednessPart:
        ...
        for rule_predicate_function in self.rule_literals:
            if rule_predicate_function != head:
                ...
                for combination in combinations:
                    ...
                    for f_arg in f_args:
                        if (
                            f_arg in h_vars and f_arg in f_vars_needed
                        ):  # Variables in head
                            associated_index = associated_variables[f_arg]
                            body_combination[f_arg] = combination[associated_index]
                            # body_combination[f_arg] = head_combination[f_arg]
                    ...
                    unfound_rule = (
                        f"{unfound_atom} :-{unfound_body} {sign_adjusted_predicate}."
                    )
                    ...
                    if self.cyclic_strategy in [
                        ...
                        if (
                            self.current_rule
                            in self.rule_strongly_restricted_components
                        ):
                            ...
                            if rule_predicate_function in relevant_bodies:
                                new_head_name = (
                                    f"{head.name}{self.current_rule_position}"
                                )
                                # new_head_name = f"{head.name}'"

                                full_head_args = [
                                    argument
                                    for argument in full_head_args
                                    if argument != ""
                                ]
                                if len(full_head_args) > 0:
                                    head_predicate = (
                                        f"{new_head_name}({','.join(full_head_args)})"
                                    )
                                else:
                                    head_predicate = f"{new_head_name}"

                                unfound_level_mapping = f"{unfound_atom} :-" +\
                                    f"{unfound_body} not prec({unfound_predicate},{head_predicate})."
                                self.printer.custom_print(unfound_level_mapping)

                                if len(full_head_args) > 0:
                                    original_head_predicate = (
                                        f"{head.name}({','.join(full_head_args)})"
                                    )
                                else:
                                ...
                                unfound_predicate_args = [
                                    argument
                                    for argument in unfound_predicate_args
                                    if argument != "_"
                                ]
    ...
    def _add_atom_to_unfoundedness_check(self, head_string, unfound_atom):
        ...
        self.unfounded_rules[head_string][str(self.current_rule_position)].append(
            unfound_atom
        )
